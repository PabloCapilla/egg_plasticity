...
gtsave(data = final_table_formatted, "./tables/Table S14.html")
...
##
##
model2_partitioning <- lmer(egg_volume_cm ~ 
                                         scale(temp_above_35) +
...
summary(model2_partitioning) # summary of model
drop1(update(object = model2_partitioning, REML = F), 
      test = "Chisq") # Likelihood-ratio test for each predictor
...
## base table
base_table <- model2_partitioning %>%
  tbl_regression(intercept = T,
                 label = list(
                   `(Intercept)` = "Intercept",
                   `scale(temp_above_35)` = 'Heat waves',
...
final_table <- base_table %>% 
  ...
  modify_table_body(fun = function(.){
    output <- left_join(x = .,
                        y = drop1_output(x=model2_partitioning) %>% 
                          dplyr::select(variable = term, Chisq=statistic, df),
                        by = "variable")
    output$df <- ifelse(output$row_type == "label",  output$df, NA)
    output$Chisq <- ifelse(output$row_type == "label",  output$Chisq, NA)
...
gtsave(data = final_table_formatted, "./tables/Table S15.html")
